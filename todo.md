...
## UI 改进（第二轮）
- [x] 日历从每月 1 号开始，不补全前面的空白天
- [x] 标题改为“Corry 教练工作日历”
- [x] 删除副标题“仅显示空闲与忙碌状态，不含具体事项”

## UI 改进（第三轮）
- [x] 底色更暖（米色偏暖黄）
- [x] 月份导航限制：只显示过去 2 个月和未来 6 个月，超出范围按鈕禁用

## 修复（第一轮）
- [x] 时区修复：ical 解析时按北京时间（UTC+8）计算忙碌时段
- [x] 左侧时间列冻结：横向拖拽时时间标签固定不动

## UI 改进（第四轮）
- [x] 在月份导航栏右侧添加刷新按钮，点击后重新拉取日历数据并显示旋转加载动画

## 修复（第二轮）
- [x] 修复全天事件时区计算：全天事件直接写入 busyHoursMap，不再走 UTC 小时遍历，修复上午时段漏标问题

## UI 改进（第五轮）
- [x] 11–12行和17–18行下方加粗横线（区分上午/下午/晚上）
- [x] 每周日列右侧加粗竖线（标记一周结束）
